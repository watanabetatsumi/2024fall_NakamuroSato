...
if (file.exists("./outputs/lm_result1.tex")) {
  file.remove("./outputs/lm_result1.tex")
}
etable(B1model_lm, B1model_glm,
       ...
       file = "./outputs/lm_result1.tex" # 出力先のファイル
       # caption = "回帰モデルの結果" # テーブルのタイトル
)
AddHeader("lm_result1")
RemoveFiles()

if (file.exists("./outputs/lm_result_factor1.tex")) {
  file.remove("./outputs/lm_result_factor1.tex")
}
etable(B1model_lm_factor, B1model_glm_factor,
       ...
       dict = c(U18_SubstanceExp = "未成年の違法使用(総合)",
                ...
                Isfemale = "女ダミー", N_siblings = "兄弟サイズ",
                MotherID = "Mother ID", BirthYear = "Birth Year"),
       file = "./outputs/lm_result_factor1.tex" # 出力先のファイル
       # caption = "回帰モデルの結果" # テーブルのタイトル
)
AddHeader("lm_result_factor1")
RemoveFiles()
...
if (file.exists("./outputs/lm_result2.tex")) {
  file.remove("./outputs/lm_result2.tex")
}
etable(B1model_lm, B1model_lm_marijuana, B1model_lm_alcohol, B1model_lm_tabaco, B1model_lm_nonegraduate,
  ...
  dict = c(U18_SubstanceExp = "未成年の違法使用(総合)", U18_mariExp = "未成年大麻経験",
           ...
           Isfemale = "女ダミー", N_siblings = "兄弟サイズ",
           MotherID = "Mother ID", BirthYear = "Birth Year"),
  file = "./outputs/lm_result2.tex" # 出力先のファイル
)
AddHeader_wide("lm_result2")
RemoveFiles()
